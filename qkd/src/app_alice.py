...
def distribute_e91_states(conn, epr_socket, socket, target, n):
    bit_flips = [None for _ in range(n)]
    basis_flips =  [random.randint(1, 3) for i in range(n)]

    for i in range(n):
        q = epr_socket.create_keep(1)[0]
        if basis_flips[i] == 1:
            q.H()
        elif basis_flips[i] == 2:  # W basis
            q.S()
            q.H()
            q.T()
            q.H()
        elif basis_flips[i] == 3:  # Z basis
            pass
        m = q.measure()
        conn.flush()
        # Synchronize with the other node so that entanglement operations
...
def main(app_config=None, num_bits=100, x=0, y=0):
    ...
    with alice:
        bit_flips, basis_flips = distribute_e91_states(
