...
def receive_e91_states(conn, epr_socket, socket, target, n):
    bit_flips = [None for _ in range(n)]
    basis_flips = [random.randint(1, 3) for _ in range(n)]
    ...
    for i in range(n):
        ...
        if basis_flips[i]==1:
            ...
            q.T()
            q.H()
        elif basis_flips[i] == 2:
            pass
        elif basis_flips[i] == 3:
            q.S()
            q.H()
            q.Z()
            q.S()
            q.T() # Tdag is not defined
            q.H()
        m = q.measure()
        conn.flush()
        # Synchronize with the other node so that entanglement operations
...
def main(app_config=None, num_bits=100, x=0, y=0):
    ...
    with bob:
        bit_flips, basis_flips = receive_e91_states(
